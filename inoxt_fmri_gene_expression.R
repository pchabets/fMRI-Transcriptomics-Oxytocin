...
######################################################################################################################################################################################
## Institution: Leiden Medical Center Utrecht ## Script: preprocessing of gene expression data and heatmaps of expression data with different configurations for inspection #####
######################################################################################################################################################################################
...
# multiple probes annotated to a gene of interest were selected from the file "reannotated_probes.csv" which contains
# a restructured list of the output of the ReAnnotator package (download package and instructions from https://sourceforge.net/projects/reannotator/)
# original outputted file is a .txt file (reannotation_ALL_[4.5.2020].txt)
...
donors <- c(1, 2, 3, 4, 5, 6) # remove the 5 for male-only analysis 
...
# read in file
chosen_mask <- "OXT-PBO_ALL_P.nii" # choose which mask, by storing the file name of the desired mask. Here, the all-task mask is used.
z <- readnii(chosen_mask) #all-tasks mask
...
L <- f.read.nifti.header(chosen_mask)
...
i <- as.data.frame(intp[intp >= 0.2]) ##threshold for including samples: at least 0.2 as interpolated mask-value, approximates 3D nearest neighbour interpolation.
...
colnames(cort_aff_oxy)[1] <- "acronym"
...
colnames(subcort_aff_oxy)[1] <- "acronym"
...
colnames(cort_unaff_oxy)[1] <- "acronym"
...
colnames(subcort_unaff_oxy)[1] <- "acronym"
...
colnames(cort_aff_vaso)[1] <- "acronym"
...
colnames(subcort_aff_vaso)[1] <- "acronym"
subcort_aff_vaso$affected <- as.factor("yes")

# means within unaffected structures for vasopressin genes
...
colnames(cort_unaff_vaso)[1] <- "acronym"
...
colnames(subcort_unaff_vaso)[1] <- "acronym"
...
hmcortoxy <- Heatmap(cortmatoxy[,1:3], name = 'Z-Score',
                  ...
                  column_names_rot = 45,
                  width = unit(ncol(cort_aff_oxy)*1.5, "lines"), 
                  height = unit(nrow(cort_aff_oxy)*0.6, "lines"),
...
hmsubcortoxy <- Heatmap(subcortmatoxy[,1:3], name = 'Subcortical\nZ-Score\nexpression',
                     ...
                     height = unit(nrow(subcort_aff_oxy)*0.58, "lines"),
...
hmcortvaso <- Heatmap(cortmatvaso[,1:3], name = 'Cortical\nZ-Score\nexpression',
                     ...
                     height = unit(nrow(cort_aff_vaso)*0.6, "lines"),
...
hmsubcortvaso <- Heatmap(subcortmatvaso[,1:3], name = 'Subcortical\nZ-Score\nexpression',
                      ...
                      height = unit(nrow(subcort_aff_vaso)*0.56, "lines"),
...
cort_aff_all <- merge(cort_aff_oxy, cort_aff_vaso, by = "acronym")
cort_aff_all <- cort_aff_all[order(cort_aff_all$OXTR),] # ordering by OXTR 
cort_unaff_all <- merge(cort_unaff_oxy, cort_unaff_vaso, by = "acronym")
...
rownames(cortmatall) <- c(cort_aff_all$acronym, cort_unaff_all$acronym)
...
subcort_aff_all <- merge(subcort_aff_oxy, subcort_aff_vaso, by = "acronym")
subcort_aff_all <- subcort_aff_all[order(subcort_aff_all$OXTR),] # ordering by OXTR 
subcort_unaff_all <- merge(subcort_unaff_oxy, subcort_unaff_vaso, by = "acronym")
...
rownames(subcortmatall) <- c(subcort_aff_all$acronym, subcort_unaff_all$acronym)
...
## to obtain all acronyms and structure names

info <- lapply(donors, function(d) { 
  sample_annot <- read.csv(paste0("normalized_microarray_donor0", d, "/SampleAnnot.csv"), header = TRUE)
  })

info <- Reduce(rbind, info) 
strucac <- info$structure_acronym
uniqueind <- unique(strucac)
indac <- vector() # to store indexes for each acronym
ind <- vector() # to store all indexes
for (i in 1:length(uniqueind)) {
  indac <- which(strucac==uniqueind[i]) 
  ind <- append(ind, indac[1]) }

acronyms <- info[ind,5:6]
